...
//						echo $this->paypal_data;
//var_dump($paypal_product);die;
	...
	public function payment_success(){
		...
		if(!empty($this->input->get('token')) && !empty($this->input->get('PayerId'))){
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr['ACK']) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
				...
				$padata = '&TOKEN='.urlencode($token);
				$httpParsedResponseAr = $this->paypal->PPHttpPost('GetExpressCheckoutDetails',$padata,$this->config->item('paypal_api_username'),$this->config->item('paypal_api_password'),$this->config->item('paypal_api_signature'),$this->config->item('paypal_api_endpoint'));
			} else {
				//show error message
				echo '<pre>';
				print_r($httpParsedResponseAr);

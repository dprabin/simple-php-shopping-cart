...
class User_model extends CI_Model {
    ...
    public function register()
    {
        ...
        $data   = array(
            'first_name'  => $this->input->post( 'first_name' ), 'last_name' => $this->input->post( 'last_name' ), 'email' => $this->input->post( 'email' ), 'username' => $this->input->post( 'username' ), 'password' => md5( $this->input->post( 'password' ) ), 'address' => $this->input->post( 'address' ), 'address2' => $this->input->post( 'address2' ), 'phone' => $this->input->post( 'phone' ), 'city' => $this->input->post( 'city' ), 'state' => $this->input->post( 'state' ), 'last_logon_ip' => $userip, 'geolocation' => $this->geoCheckIP( $userip ),//use this only in live site
    ...
    public function edit_user( $id )
    {
        ...
        $userip = $this->find_user_ip();
        $data   = array(
            'address' => $this->input->post( 'address' ), 'address2' => $this->input->post( 'address2' ), 'phone' => $this->input->post( 'phone' ), 'city' => $this->input->post( 'city' ), 'state' => $this->input->post( 'state' ), 'email' => $this->input->post( 'email' )
        );

        if ( $this->session->userdata( 'user_id' ) == $id )
        { //if it is edited by user
        ...
        { //update password only if it is supplied
            $data[ 'password' ] = md5( $this->input->post( 'password' ) );
        }

        if ( $this->input->post( 'previllege' ) )
        {
            $data[ 'previllege' ] = $this->input->post( 'previllege' );
		}
